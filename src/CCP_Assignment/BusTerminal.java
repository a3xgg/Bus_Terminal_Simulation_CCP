    ...
    public void enterBusTerminal(Customer customer){
        synchronized (this){
            ...
            if(!max){
                ...
                System.out.println("(" + java.time.LocalTime.now().withNano(0) + " - " + getBusTerminalName() + ")\t" + "Customer " + customer.getName() + " reached the terminal entrance");
                try{
                    Thread.sleep(1000);
                } catch(Exception e){}
                customer.entered = true;
                System.out.println("(" + java.time.LocalTime.now().withNano(0) + " - " + getBusTerminalName() + ")\t" + "Customer " + customer.getName() + " has entered the bus terminal" + "(Current Capacity: " + customerCounter+")");
            } else {
                System.out.println("(" + java.time.LocalTime.now().withNano(0) + " - " + getBusTerminalName() + ")\t" + "Customer " + customer.getName() + " is currently waiting at the entrance");
                try{
                    Thread.sleep(5000);
                } catch(Exception e){}
            }
